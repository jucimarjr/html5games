var GameEndWinLayer = cc.Layer.extend({
    init:function()
    {
        this._super();
        ...
        var fundo = cc.Sprite.create("assets/Telas/ganhou.png");
        fundo.setPositionX(screen.width / 2);
        fundo.setPositionY(screen.height / 2);
        this.addChild(fundo);

        var finalScore = sys.localStorage.getItem("finalScore");

        var Label = cc.LabelTTF.create("Voce atingiu " + finalScore.toString() + " fantasmas.", "Grinched", 50);
...
var GameEndWinScene = cc.Scene.extend({
    onEnterTransitionDidFinish:function(){
        this._super();
        var layer = new GameEndWinLayer();
        layer.init();
        this.addChild(layer);
    }
});